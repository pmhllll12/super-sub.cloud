...
**Goal:** 「선수와 비교하기」에서 두 영상의 관절을 뽑아 세 순간(직전 · 임팩트 · +1초)의 뼈대를 겹쳐 보여 주고, 각도 차이를 규칙 문장 요약으로 붙인다.

**Architecture:** 관절은 브라우저가 보이지 않는 `<video>` 를 넘기며 기존 검출기로 뽑는다(`lib/motion/extract.ts`). 세 순간 · 정렬 · 각도는 순수 함수(`moments.ts` · `align.ts` · `angles.ts`)라 가짜 관절로 시험한다. 화면은 `source.ts` 하나만 보므로 최종에 서버 값으로 바꿀 때 그 파일만 고친다. 요약은 코드 규칙 문장이다(나중에 Gemini 를 얹을 자리는 `SummaryRequest`).

**Tech Stack:** Next.js 16 (App Router, `www/`) · React 19 · TypeScript · Vitest + Testing Library(jsdom) · 기존 MoveNet 검출기(`lib/personDetector.ts`)
...
- 요약은 **코드 규칙 문장**(`describe.ts`, 2026-09-14 결정) — 서버 · Gemini 를 부르지 않는다. 차이 5° 미만은 「비슷합니다」.
- 화면 표기: 「브라우저에서 잰 값」 · 「데모 영상(Pexels)」.
...
- 🔴 **정정 (2026-09-15)** — 이 문서는 계획 당시의 기록이라 그대로 두지만, 위 「직전 = 임팩트 앞 0.5초 안 무릎각 최소」는 그 뒤 바뀌었다. 지금 정의(임팩트 0.3초 전, 없으면 가장 가까운 잡힌 프레임)는 설계 문서와 미결 `paik` 30번을 따른다.
...
| Create `src/lib/motion/describe.ts` | 각도 차이를 규칙 문장으로 |
| Create `src/components/analysis/CompareMoments.tsx` | 세 순간 카드(겹친 뼈대) |
| Create `src/components/analysis/CompareSummary.tsx` | 오른쪽 판의 비교 요약 절(규칙 문장) |
...
### Task 6: (뺐다 — 2026-09-14 결정)

사용자 결정: 요약은 **Gemini 없이 코드 규칙 문장**으로 만든다. 서버 라우트가 필요 없어 이 과제는
하지 않는다. 나중에 Gemini 를 얹을 때는 `SummaryRequest`(순간별 각도 표)를 그대로 넘기는 층을
더하면 되고, 그때 쓸 응답 파싱 · 숫자 검사는 Task 4 에 이미 있다.
...
**Files:**
- Create: `src/lib/motion/describe.ts`
- Create: `src/components/analysis/CompareSummary.tsx`
- Test: `src/lib/motion/describe.test.ts`
- Test: `src/components/analysis/CompareSummary.test.tsx`

**Interfaces:**
- Consumes: `SummaryRequest` from `@/lib/motion/summaryContract`; `MOMENT_LABEL`, `MetricId`, `MetricRow`, `MomentKey` from `@/lib/motion/types`
- Produces:
  - `SIMILAR_DEG = 5`, `type ComparisonText = { moments: { key: MomentKey; lines: string[] }[]; summary: string }`, `describeLine(row: MetricRow): string`, `describeComparison(req: SummaryRequest): ComparisonText`
  - `<CompareSummary playerName request failedReason onSelect />` — `request: SummaryRequest | null`, `failedReason: string | null`, `onSelect: (key: MomentKey) => void`. 서버를 부르지 않는다.

- [ ] **Step 1: 규칙 문장 시험을 먼저 쓴다**

`src/lib/motion/describe.test.ts`:
...
Run: `npx vitest run src/lib/motion/describe.test.ts`
Expected: FAIL — `Failed to resolve import "./describe"`

- [ ] **Step 3: 규칙 문장을 구현한다**

`src/lib/motion/describe.ts`:
...
Run: `npx vitest run src/lib/motion/describe.test.ts src/components/analysis/CompareSummary.test.tsx && npx tsc --noEmit`
Expected: PASS (8 tests) · `tsc` 출력 없음

- [ ] **Step 9: 커밋**

```bash
git add src/lib/motion/describe.ts src/lib/motion/describe.test.ts src/components/analysis/CompareSummary.tsx src/components/analysis/CompareSummary.test.tsx
git commit -m "feat(www): 두 자세의 각도 차이를 규칙 문장으로 요약한다"
```
...
git commit -m "feat(www): 선수를 고르면 세 순간 비교와 요약이 선다"
```
...
`www/docs/2026-08-28-작업-현황.md` 맨 위(`## 1.8 …` 앞)에 `## 1.9 이번 회차(2026-09-14) — 세 순간 자세 비교` 절을 넣는다 — 무엇이 생겼나(표) · 🔴 데모인 곳(브라우저 값 · Pexels 영상 · 교체 지점 `source.ts`) · Step 2 의 실측값 · 아직 안 된 것(미결 paik 29 · 30 · Gemini 코칭 문장은 나중에 얹을 자리).
