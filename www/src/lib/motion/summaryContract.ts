/**
 * 비교 요약이 쓰는 **요청 모양과, 그걸 채우는 셈**. 🔴 **정정 (2026-09-14)** — 처음엔
 * 이 요청을 Gemini 서버 라우트에 보낼 계획이었으나, 지금 화면은 `lib/motion/describe.ts`
 * 의 **코드 규칙 문장**을 쓴다(서버를 안 부른다 — `www/docs/2026-09-14-선수-비교-분석-design.md`
 * §4). `buildSummaryRequest` 가 만드는 순간별 각도표(`SummaryRequest`)는 그 규칙 함수의
 * 입력으로 그대로 쓰이고, 이 파일의 응답 파싱 · 숫자 검사(`parseSummaryResponse` ·
 * `isGrounded`)는 **나중에 Gemini 코칭 문장 층을 얹을 때**를 위해 남겨 둔 것이다 — 지금은
 * 안 쓰인다.
 *
 * 🔴 보내는(만드는) 것은 **숫자와 항목 이름뿐**이다(영상 · 사진 · 사용자 정보 없음) — 나중에
 * Gemini 에 실제로 보낼 때도 이 성질은 그대로 지킨다.
 */
