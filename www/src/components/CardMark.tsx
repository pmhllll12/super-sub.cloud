 ...
 * 카드 위에서는 갈린다.
 *
 * 🔴 **13~21 은 2026-09-18 에 아홉을 더한 것**이다(사용자 요청). 손으로 그린
 * 그림이 아니라 `scripts/marks.html` 을 `scripts/bake-marks.py` 가 구워 낸
 * 것이라 — **PNG 를 직접 고치면 다음에 구울 때 조용히 덮인다.**
 * ⚠️ 그중 **글리치 스펙트럼 · 홀로그램 레이어 · 유동 금속 · 파티클 플럭스는
 * 원본이 색으로 읽히는 것**(무지개 · 크롬 반사 · 검은 바탕의 반짝임)이라
 * **구조만** 남겼다(사용자 판단). 마스크는 알파만 쓰므로 색을 넣을 자리가
 * 없고, 색을 살리려면 「자국 색」이 안 먹는 갈래를 따로 내야 한다.
 */
export const MARKS = [
  '기본',
  '없음',
