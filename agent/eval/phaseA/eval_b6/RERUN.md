...
> 🔴 **`git.dirty` 를 곧이곧대로 읽지 말 것 (2026-09-21, 47번 6회차).**
> 이 칸은 **자기가 방금 덮어쓴 CSV 를 자기가 보고** 참이 된다 — 산출이
> 달라지는 실행에서는 **거의 항상 `true`** 다. 09-15 실행이 `false` 였던 것은
> 그날 산출이 **비트 동일**이라 git 이 변경으로 안 본 우연이다.
> `dirty_scope` 를 함께 볼 것: **`src`·`rubrics` 가 들어 있지 않으면**
> 코드는 깨끗하고 더러운 것은 산출물뿐이다. (자기 산출 3개를 검사에서 빼는
> 것이 맞고, 별도 회차로 둔다.)
...
| (아래 참조) | **2026-09-15** | **미결 43번 ㉳ 3회차** — 마무리 길이를 구간 안에서 센다. 커밋은 `git log -- selector_downstream_comparison.csv` 로 찾는다. 🔴 **이 산출은 정본이 아니었다** — 아래 09-21 참조 |
| (아래 참조) | **2026-09-21** | 🔴 **기준선 재선언** (미결 47번 (나)). 09-15 산출이 **`torchvision` 이 빠진 환경(S0)** 의 것이었다 — 그때 `run_meta.json` 에 `torchvision` 칸이 아예 없는 것이 증거다. 49번이 **`torchvision` 경로로 통일**을 선언한 뒤 같은 스크립트를 그 환경에서 다시 돌렸다. Track 2 는 **09-08 기준선과 불일치 0**(다섯 번째 재현), Track 1 은 **수치 37열 비트 동일**이고 `fail_reason` 문구만 `97e8f323`(미결 41번) 기준으로 바뀌었다. 사전 등록 `pending47_baseline_audit/PREREGISTRATION_6.md` · 결과 `RESULTS_6.md` · 덮어쓰기 전 산출은 같은 폴더 `pre_redeclare_*` |
