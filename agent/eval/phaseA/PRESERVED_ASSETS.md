...
> ## 2026-09-16 — **저장소에 못 넣는 자산에 두 번째 사본과 지문이 생겼다**
>
> `clips/`(130MB)·`labeling/`(31MB)은 저장소에 넣을 성질이 아니라 **한 벌**로
> 살아 있었다(미결 11번). 이제 둘이고, **어느 쪽이든 내용으로 확인된다.**
>
> | | |
> |---|---|
> | 두 번째 사본 | `~/supersub-assets/phaseA/` (ext4 — `/mnt/d` 는 Windows drvfs 다) |
> | 지문 | `preserved_manifest.csv` — **198개 파일의 path·bytes·md5** (15KB, 저장소 안) |
> | 확인 | `uv run python eval/phaseA/verify_preserved.py [--root <사본>]` · 종료 코드 0/1 |
> | 대조 결과 | 두 사본 **198/198 md5 일치**, 없는 루트를 주면 198개 어긋남으로 나온다(판별이 되는 검사다) |
>
> 🔴 **왜 지문이 먼저인가.** 사본을 늘리는 것만으로는 「그때 그 바이트인가」에
> 답하지 못한다. 미결 47번이 **일주일**을 쓴 질문이 정확히 그것이었고, 그때
> 없던 것이 이 매니페스트다. 🔴 **어긋나면 덮어쓰기 전에 어느 쪽이 맞는지부터
> 정한다** — 다른 사본이 틀렸을 수도, 매니페스트가 낡았을 수도 있다.
>
> 🔴 **같은 기계의 다른 파일시스템이라 디스크 고장은 못 막는다.** 막는 것은
> Windows 쪽에서 드라이브를 지우거나 옮기는 것, drvfs 가 안 붙는 상황,
> 실수로 한 벌을 지우는 것이다. 기계 밖 사본은 아직 없다.
