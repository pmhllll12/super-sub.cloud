...
## 만들어진 리소스 (2026-09-02, 계정 0706-0555-3723 / 서울)

뒤 단계에서 골라야 하는 값들이다. 콘솔에서 이름으로 찾아도 되지만, ID가 있으면
헷갈릴 일이 없다. **비밀값이 아니다** — 계정 안에서만 의미가 있는 식별자다.
...
| 인터넷 게이트웨이 | `igw-04cef37962f4b2ed8` |
...
| EC2 인스턴스 | `i-0ea81b71b57c047ff` (`supersub-ai`, g4dn.xlarge, ap-northeast-2a) — 2026-09-03 생성 |

**퍼블릭 IP는 여기 적지 않는다.** 중지·시작할 때마다 바뀌므로(2026-09-03 첫 기동은
`13.125.107.138`) 적어 두면 반드시 틀린 값이 된다. 콘솔의 인스턴스 상세 화면에서
그때그때 본다.

> 같은 계정에 `supersub`(`i-0f50dda42eb3c9f31`, t3.large, 2b)가 따로 떠 있다.
