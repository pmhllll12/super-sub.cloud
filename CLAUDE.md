...
- **`main` 통합은 박민호가 맡습니다.** 각 사람 브랜치를 `main`에 병합하고, 필요하면
  **`main`을 각 브랜치로 되병합(sync)** 해서 다른 사람 작업·결정이 흘러가게 합니다.
  그래서 **`origin/<내 브랜치>`가 내가 push하지 않았는데도 앞서 있을 수 있습니다** —
  박민호의 sync 병합입니다.
...
- 🔴 **남의 브랜치를 되병합할 때는 추가만 합니다** — 그 사람 커밋을 rebase하거나
  force-push로 덮지 않습니다. 커밋 해시가 미결 항목·설계 문서에서 참조되고,
  덮으면 그 참조가 끊깁니다. sync 병합은 `--no-ff`로, 남의 커밋은 그대로 둡니다.
...
**시점은 스프린트 종료 시점입니다** — 위 「스프린트 로드맵 & 칸반」 절의
"스프린트가 끝나면 칸반을 다음 스프린트로 교체" 타이밍에 얹습니다. 압축(제자리
줄이기)은 그때그때 자유지만, **다른 파일로 옮기는 것은 스프린트가 끝날 때 한
번에** 합니다 — 매번 옮기면 신경 쓸 일이 늘고, 진행 중인 스프린트 안에서
번호·위치가 흔들리면 그 스프린트를 같이 보는 사람에게 방해가 됩니다.
