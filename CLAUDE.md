...
| 쓰지 않습니다 | 자리표시자 |
|---|---|
| AWS 계정 ID (12자리 · `arn:aws:...::` 안 포함) | `<AWS 계정 ID>` |
| 서버·탄력적·과거 **공인 IP** | `<서버 IP>` |
| VPC·서브넷·보안그룹·인스턴스·EIP 할당 ID (`vpc-`·`subnet-`·`sg-`·`i-`·`eipalloc-`) | `<VPC ID>` 등 |
| IAM 역할·사용자·정책 이름 | `<EC2 역할>` 등 |
| S3 버킷 이름 | `<S3 버킷>` |

리전(`ap-northeast-2`)·서비스 도메인(`api.supersub-ai.com`)처럼 **어차피 공개인 것**은
그대로 씁니다. 구분 기준은 "이 값이 공격자에게 무언가를 주는가"입니다.

🔴 **커밋 전에 돌립니다** (걸리면 자리표시자로 바꿉니다):

```bash
git diff --cached --name-only -- jekyll/ _posts/ guide/ \
  | xargs grep -nE 'arn:aws:[a-z]+::[0-9]{12}|(vpc|subnet|sg|eipalloc)-[0-9a-f]{8,}|\bi-0[0-9a-f]{16}\b|\b(3|13|15|43|52|54)\.[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\b' 2>/dev/null
```
