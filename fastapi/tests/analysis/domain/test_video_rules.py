...
class TestRejectReason:
    ...
    def test_해상도는_상한이_없다(self):
        """결정(2026-09-19, 박민호 — `jin` 43번) — 해상도 상한 자체를 없앴다.
        메모리 예산은 agent/ 쪽 바이트 가드가 해상도와 무관하게 스스로
        지킨다. 방향·크기와 무관하게 전부 통과해야 한다."""
        assert reject_reason(**_ok(width=3840, height=2160)) is None  # 4K 가로
        assert reject_reason(**_ok(width=2160, height=3840)) is None  # 4K 세로
        assert reject_reason(**_ok(width=1080, height=1920)) is None  # 세로 1080p
        assert reject_reason(**_ok(width=7680, height=4320)) is None  # 8K
        assert reject_reason(**_ok(width=2160, height=4096)) is None  # DCI 4K 세로
