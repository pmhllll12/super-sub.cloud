...
**내 기록은 지우고, 남에게 남긴 기록은 작성자만 비운다**(2026-09-17). 경기 지원·
불참·크레딧·코치 연결·스쿼드 등재, 그리고 **나에 대한** 평가·신고는 함께 지워진다.
**내가 남에게 쓴** 평가와 **내가 한** 신고는 상대의 신뢰 등급·제재 근거라 남기고
작성자만 비운다(익명). 경기·팀·스쿼드·코치처럼 여럿이 쓰는 데이터는 남는다.

> 🔴 **정정** — 이 날 전까지는 위 기록이 한 건이라도 있는 사람의 탈퇴가 DB 에서
> 거부되어 `500` 이 났다(외래키에 삭제 규칙이 없었다). 응답 모양은 바뀌지 않았고
> 이제 `204` 로 끝난다.
...
### `DELETE /api/v1/me/card` — 카드를 안 만든 처음 상태로 (2026-09-19 추가, 백성검)

인증 필요. 본문 없음. **내 카드를 지운다.** 화면의 「초기화」가 부른다 —
카드를 **안 만든 처음 상태**(기본 빈 카드 · 「카드를 먼저 만들어주세요」 안내)로
돌아간다(사용자 요청).

| 응답 | 뜻 |
|---|---|
| `204 No Content` | 지웠다. 🔴 **원래 없었어도 204** — 멱등이다(재시도가 오류로 오지 않게) |
| 401 `UNAUTHORIZED` | 로그인이 필요하다 |

🔴 **되돌릴 수 없다.**
- 공개 주소(`public_slug`)가 사라진다 — 이미 공유한 `GET /cards/{slug}` 가 **404** 가 된다
- 스쿼드 판의 자리(`squad_member`)가 **같이 빠진다**(외래키 `ondelete="CASCADE"`)
- **호칭은 남는다** — 부여된 것(`user_title`)도 직접 적은 것(`user_custom_title`)도
  카드가 아니라 **사람**(`user`)에 붙어 있어서, 다시 만든 카드에 그대로 실린다
- 카드 사진(S3 `cards/…`)은 지우지 않는다 — 가리키는 키가 사라질 뿐이다

다시 `POST /me/card` 하면 **새 카드**가 생긴다(201, 슬러그도 새로 뽑힌다).

마이그레이션 없음 — 행 삭제뿐이고 외래키 규칙은 이미 걸려 있었다.
...
GPU 인스턴스가 꺼져 있을 수 있어 즉시 응답하는 동기 API로는 못 둔다(같은
이유로 `analyze` 작업도 폴링이다).

> 🔴 **정정 (2026-09-18)**: 앞서 이 자리에 「`agent-ai` autostop 때문에」라고
> 적었는데, **그 오토스탑은 팀 결정으로 없앴다**(`a16848b`). 이제는 사람이 꺼
> 두는 경우라 **결론은 같다** — 근거만 낡았고 설계는 안 바뀐다.
...
### 상한 (2026-09-03 결정, 해상도는 2026-09-19 철회, 개수는 2026-09-22 신설)

| 항목 | 값 |
|---|---|
| 용량 | 200MB |
| 길이 | 60초 |
| 해상도 | **상한 없음** (2026-09-19 결정 — 아래 참고) |
| 형식 | `video/mp4` · `video/quicktime` |
| **개수** | **갈래마다 3개** — 분석 영상 3 · 업로드 영상 3 (2026-09-22 신설 — 아래 「갈래마다 3개」 절) |

🔴 **정정 (2026-09-19, 박민호 결정 — CCC 63번대 참고)**: 해상도 상한을 없앴다.
`ho` 9번에서 메모리 가드가 장수(300)가 아니라 **바이트**로 옮겨진 뒤로는
해상도가 커질수록 분석 프레임 수가 스스로 줄어 예산을 지킨다 — 그래서 여기서
긴 변·짧은 변을 따로 재는 관문 자체가 더는 필요 없었다. DCI 4K 세로
(2160×4096) 영상이 이 관문 때문에 반려되는 것을 사용자가 직접 겪고 결정했다.
`analyze` 가 참이든 거짓이든 이제 해상도는 안 본다. 용량·길이는 그대로다
(용량·길이는 `analyze` 와 무관하게 항상 본다).

🔴 **이전 정정 (2026-09-11, 이제는 지난 일)**: 한때 상한이 1920x1080에서
4K(2160×3840)로 오른 적이 있었다 — 그마저도 위 결정으로 없어졌다.
...
### 🔴 갈래마다 3개 — `VIDEO_LIMIT_EXCEEDED` (2026-09-22 신설, 사용자 요청)

영상은 **갈래마다 3개**까지다. 위 용량·길이와 달리 **클립 하나가 아니라 계정을
재는 값**이라, 클라이언트가 알아야 할 것이 셋 더 있다.

**갈래는 화면의 두 탭과 같다.** 가르는 기준은 **`analysis_job_id` 유무**이고,
`www` 의 「내 영상」이 이미 같은 기준으로 탭을 가른다.

| 갈래 | 무엇이 여기 서나 | 상한 |
|---|---|---|
| **분석 영상** (`analysis_job_id` 있음) | 분석을 걸고 「내 프로필에 저장」한 것 | 3 |
| **업로드 영상** (`analysis_job_id` 없음) | `analyze: false` 기록용 · **반려된 클립** · 중복 재사용 | 3 |

🔴 **합쳐서 3개가 아니다.** 한 계정의 최대는 **3 + 3 = 6개**이고, **한 갈래가
차도 다른 갈래는 그대로 열려 있다.**

**무엇을 세나 — `GET /videos` 가 돌려주는 것**(`kept=true`)만이다.

| | 자리를 차지하나 |
|---|---|
| 「내 프로필에 저장」한 영상 | **예** (분석 갈래) |
| `analyze: false` 로 올린 기록용 영상 | **예** (업로드 갈래) |
| **반려된 클립**(`passed: false`) | **예** (업로드 갈래 — 분석 작업이 안 생긴다) |
| 분석 중인 임시 클립(`kept: false`) | 아니오 — 화면에도 안 보이고 스윕이 걷어 간다 |
| 남의 영상 | 아니오 |

**어디서 막히나 — 세 경로 전부.** 하나만 막으면 새는 자리가 있어서다.

| 경로 | 왜 여기서도 |
|---|---|
| `POST /videos/upload-url` | 올리기 전에 막는다 — 200MB 를 다 올린 뒤 막히면 그 대역폭이 버려진다 |
| `POST /videos` | 자리가 있을 때 키를 미리 받아 두면 (1) 을 건너뛸 수 있다. **갈래가 여기서 정해진다** |
| `POST /videos/{video_id}/keep` | 자리가 있을 때 등록된 임시 클립 여럿이 **나중에 한꺼번에** 저장되는 경우 |
...
`message` 는 **막힌 갈래 이름**(「분석 영상」/「업로드 영상」)을 담는다 — 화면의
탭 이름과 같은 말이다. `code` 는 갈래와 무관하게 하나다.

🔴 **반려(`passed: false`)가 아니라 `422` 다.** 위 「반려는 실패가 아니다」는
"등록은 됐고 분석 대상이 아닐 뿐"인 경우인데, 개수 초과는 **등록 자체가 성립하지
않는다** — 반려 행으로 남기면 그 행이 **업로드 갈래에서 또 한 자리**를 차지해
상황이 더 나빠진다. 그래서 클라이언트는 이것만은 **상태 코드·`code` 로 분기**한다.

**푸는 법은 `DELETE /videos/{video_id}` 하나다.** 그 갈래의 자리가 즉시 빈다.

⚠️ **이미 상한을 넘게 가진 계정은 그대로 둔다** — 이 상한은 **새로 늘리는 것만**
막는다. 기존 영상을 지우게 하지 않는다.

#### `upload-url` 의 `analyze` 는 **어느 갈래인지 알려 주는 힌트**다

`POST /videos/upload-url` 은 갈래를 모른다 — 그것은 등록할 때 `analyze` 와 규격
검사로 정해진다. 그래서 **선택 필드 `analyze`** 를 받는다.

| 보냈나 | 이 호출이 하는 일 |
|---|---|
| **보냈다** | 그 갈래만 보고 찼으면 `422` |
| 안 보냈다 | **양쪽이 다 찼을 때만** `422` |

🔴 **안 보내도 동작은 같다** — 등록에서 어차피 막힌다. 다만 한쪽만 찬 상태에서
그 갈래로 올리려던 사람은 **200MB 를 다 올리고 나서** 막힌다. 보내면 그 헛걸음이
없어진다. `POST /videos` 에 보낼 값과 같은 값을 그대로 주면 된다.

🔴 **안 보냈을 때 한쪽만 찼다고 막지 않는다** — 안 찬 갈래로 올리려는 사람을
잘못 막게 되기 때문이다. 모를 때는 관대한 쪽으로 기운다.
...
| 422 | `VIDEO_LIMIT_EXCEEDED` | 그 갈래가 이미 **3개**다 — `analyze` 를 줬으면 그 갈래, 안 줬으면 양쪽이 다 찼을 때 (2026-09-22 추가 — 위 절) |
...
반려면 `passed: false` · `reject_reason: "용량이 상한을 넘습니다: 210MB (상한
200MB)"` (또는 길이 초과 문장) · `analysis_job_id: null` 이다. **반려된
클립은 분석하지 않는다** — 규격 검사를 두는 이유가 그것이다. 🔴 해상도는
더 이상 반려 사유가 아니다(2026-09-19 결정, 위 「상한」 절 참고).
...
| 422 | `VIDEO_LIMIT_EXCEEDED` | 이 클립이 설 갈래가 이미 **3개**다 (2026-09-22 추가 — 위 「갈래마다 3개」 절) |
| 503 | `STORAGE_NOT_CONFIGURED` | 서버에 `S3_BUCKET` 이 없다 |
...
### `GET /api/v1/videos/{video_id}/poster` — 카드에 깔 한 장면 (2026-09-24 추가, 백성검)

홈의 영상 줄이 쓴다. **JPEG 한 장**(`image/jpeg`, 가로 640). 권한은
`playback-url` 과 같다 — **공개 클립이거나 자기 클립일 때만.**

🔴 **왜 생겼나.** 목록이 썸네일을 안 실어서 클라이언트가 카드마다 **원본 MP4 를
열어** 첫 프레임을 뽑고 있었다 — 실기기에서 **한 장에 1.9초**였고, 다섯 장을
띄우는 홈이 한참 까맸다. 작은 JPEG 하나로 바꾸면 그 값이 사라진다.

- 영상의 **1초 지점**을 뜬다. 🔴 **0초가 아니다** — 시작 몇 프레임이 검거나
  페이드인인 영상이 많다
- **영상마다 딱 한 번만 비싸다.** 처음 부른 사람에게서 뜨고 캐시되며, 그 뒤로는
  모두에게 즉시 나간다. `Cache-Control: public, max-age=86400`
- ⚠️ **캐시는 컨테이너 안(`/tmp`)이다** — 🔴 **재배포하면 빈다.** 배포 뒤에 공개
  목록을 한 번 훑어 미리 채워 두면 아무도 느린 첫 요청을 안 겪는다
- 🔴 **DB·S3 에 안 둔 까닭**: DB 는 마이그레이션이 필요한데 `fastapi/CLAUDE.md` 가
  마이그레이션 체인을 공유 파일로 묶어 뒀고, S3 는 새 접두사에 쓰려면 **IAM 정책을
  고쳐야 한다**(2026-09-18 에 `cards/` 로 똑같이 막혔다). 둘 다 이 작업 범위 밖이라
  피했다. 🔴 **영구 캐시가 필요해지면 그때 둘 중 하나를 연다**
- 뜨는 방식은 `ffmpeg` 에 **사전 서명 GET 주소를 그대로 물린다** — 서버가 원본을
  내려받지 않고, **새 S3 권한도 필요 없다**(재생 주소를 만들던 그 권한이다)

| 에러 | code | 언제 |
|---|---|---|
| 404 | `VIDEO_NOT_FOUND` | 없는 클립이거나 비공개 남의 클립이다 |
| 404 | `POSTER_NOT_AVAILABLE` | 장면을 못 떴다(형식·길이, 또는 이미지에 `ffmpeg` 이 없다) |
...
- 🔴 **개수 상한(갈래마다 3개)이 여기에도 걸린다** (2026-09-22 신설 — 위 「갈래마다
  3개」 절). 갈래는 이 클립이 설 탭 그대로다(`analysis_job_id` 유무). 이 호출이 `kept=false` → `true` 를 만드는 자리라 **불변식을 지키는
  진짜 관문**이다: 자리가 있을 때 등록해 둔 임시 클립 여럿을 나중에 한꺼번에
  저장하면 앞의 두 관문을 다 통과하고도 상한을 넘기 때문이다. **이미 저장된
  클립을 다시 부르는 것은 막지 않는다** — 위 멱등이 깨지면 안 된다.
...
`formation` 은 홈 스쿼드 판의 판 크기다(`"3:3"`·`"5:5"`·`"7:7"`).
`PATCH /teams/{team_id}/squad` 로 바꾼다(아래).

🔴 **새로 만드는 스쿼드는 `"5:5"` 를 갖고 시작한다 (2026-09-18에 바뀌었다).**
전에는 `null` 로 만들고 크기 단추를 **눌러 바꿀 때만** 저장했는데, 화면은 값이
없어도 기본 판을 켜진 것처럼 그린다 — 이미 켜져 보이니 아무도 누르지 않았고
실제 DB 는 거의 전부 `null` 이었다. 팀 매칭의 첫 하드 필터가 `formation` 동등
비교라(3-13절 「맞는 상대」) **어느 팀에게도 상대가 안 잡혔다**(운영 7팀 중
6팀). 값을 지어내는 것이 아니라 화면이 내내 보여 주던 값을 저장만 한다.

⚠️ **읽는 쪽은 여전히 `null` 을 만날 수 있다** — 2026-09-18 이전에 만들어진
스쿼드다. 「모르는 값이면 기본 판으로 연다」는 그대로 지킨다.
...
#### 🔴 같은 상대에 **겹쳐 걸 수 없다** (2026-09-18 추가)

두 팀 사이에 **살아 있는 신청**이 있으면 `409 TEAM_MATCH_REQUEST_ALREADY_LIVE`.
「살아 있다」 = `pending`(답을 기다림) 이거나, `accepted` 이면서 **경기가 아직
있고**(`match_id` 가 차 있고) **아직 안 지난 것**. **거절·취소된 것, 이미 지난
경기, 물린 경기는 세지 않는다** — 안 그러면 한 번 붙은 팀과 다시는 못 붙는다.

> 🔴 **물린 경기를 가려내는 것은 `status` 가 아니라 `match_id` 다.** 경기 취소
> (`DELETE /matches/{id}`)는 `match` 행만 지우고 신청의 `status` 는 `accepted`
> 로 남는다 — FK 가 `SET NULL` 이라 `match_id` 만 비워진다. 「경기 잡힘」 화면도
> 같은 조건을 쓴다.

🔴 **방향을 가리지 않는다.** A→B 가 살아 있으면 B→A 도 막힌다 — 「이 상대와
이미 잡혀 있다」는 누가 걸었는지와 무관하다.

> **왜 생겼나**: 위 「동시 확정 방지」가 정리하는 것은 `pending` 까지다.
> `accepted` 가 된 뒤에 같은 상대에 새로 거는 것은 아무도 막지 않았고, 화면의
> 「한 번에 한 곳」은 그 브라우저의 상태뿐이라 새로고침하면 그대로 뚫렸다.
> 운영에서 한 팀이 같은 상대와 **확정 경기 3건**을 갖게 됐고, 경기를 하나
> 끝내거나 취소할 때마다 다음 것이 올라와 **「끝냈는데 또 그 화면이 뜬다」**로
> 보였다.

**여러 팀에 동시에 거는 것은 여전히 된다** — 막는 것은 *같은 상대에 두 번*이다.

| 에러 | code |
|---|---|
| 403 | `FORBIDDEN` — 신청 팀 주장이 아니다 |
| 404 | `TEAM_NOT_FOUND` — 신청 팀·대상 팀 어느 쪽이든 |
| 409 | `TEAM_MATCH_REQUEST_ALREADY_LIVE` — 그 상대와 이미 살아 있다 (위) |
