...
> **상태:** 진행 전 — 명세 확정, **구현 안 됨** · 2026-08-25 확인
...
> 비밀번호는 **8자 이상**만 본다. 5장 요구사항이 비어 있어 근거가 없으므로 최소한만
> 걸었다. 대문자·특수문자 강제는 넣지 않는다 — 사용자를 예측 가능한 패턴으로 몰 뿐이다.
> 해시는 bcrypt를 쓴다.
...
1. **DDL을 쓰고 실제 PostgreSQL에 올려 확인한다.** 부록 D는 문서일 뿐이고
   D.6 삭제 연쇄·D.7 유일제약이 아직 코드가 아니다. 로컬에 올려보면 바로 판별된다.
2. 이 문서대로 Pydantic 모델과 라우트를 만든다. FastAPI가 `/docs`에 OpenAPI를
   자동 생성하므로 **정어진은 그걸 보고 붙이면 된다.**
3. DB가 아직 없으므로 **초기에는 고정 응답(스텁)으로 먼저 열어준다.** 정어진이
   09.01에 바로 화면을 붙일 수 있게 하는 것이 목적이고, 실제 조회는 그 뒤에 채운다.
4. 정어진에게 이 문서를 공유하고 **`titles`가 빈 배열인 상태의 화면 처리**를 확인한다
   (4장 참고 — 부정 표식이 되면 안 된다).
