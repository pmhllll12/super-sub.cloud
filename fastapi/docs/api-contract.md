...
| **계정당 개수** | **3개** (2026-09-22 신설 — 아래 「계정당 3개」 절) |
...
### 🔴 계정당 3개 — `VIDEO_LIMIT_EXCEEDED` (2026-09-22 신설, 사용자 요청)

한 계정이 가질 수 있는 영상은 **3개**다. 위 용량·길이와 달리 **클립 하나가
아니라 계정 전체를 재는 값**이라, 클라이언트가 알아야 할 것이 셋 더 있다.

**무엇을 세나 — 「업로드 영상」 목록에 보이는 것 전부.** 정확히는
`GET /videos` 가 돌려주는 것과 같은 집합(`kept=true`)이다.

| | 자리를 차지하나 |
|---|---|
| 「내 프로필에 저장」한 영상 | **예** |
| `analyze: false` 로 올린 기록용 영상 | **예** (등록하는 순간 저장된 것으로 남는다) |
| **반려된 클립**(`passed: false`) | **예** — 목록에 사유와 함께 남으므로 한 자리를 쓴다 |
...
| `POST /videos` | 자리가 있을 때 키를 미리 받아 두면 (1) 을 건너뛸 수 있다 |
...
않는다** — 반려 행으로 남기면 그 행이 또 한 자리를 차지해 상황이 더 나빠진다.
그래서 클라이언트는 이것만은 **상태 코드·`code` 로 분기**한다.

**푸는 법은 `DELETE /videos/{video_id}` 하나다.** 지우면 그 자리는 즉시 빈다.

⚠️ **이미 3개를 넘게 가진 계정은 그대로 둔다** — 이 상한은 **새로 늘리는 것만**
...
| 422 | `VIDEO_LIMIT_EXCEEDED` | 저장된 영상이 이미 **3개**다 (2026-09-22 추가 — 위 절) |
...
| 422 | `VIDEO_LIMIT_EXCEEDED` | 저장된 영상이 이미 **3개**다 (2026-09-22 추가 — 위 「계정당 3개」 절) |
| 503 | `STORAGE_NOT_CONFIGURED` | 서버에 `S3_BUCKET` 이 없다 |
...
- 🔴 **계정당 개수 상한(3개)이 여기에도 걸린다** (2026-09-22 신설 — 위 「계정당
  3개」 절). 이 호출이 `kept=false` → `true` 를 만드는 자리라 **불변식을 지키는
