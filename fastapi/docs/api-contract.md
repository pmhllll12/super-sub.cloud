...
- 🔴 **지원 취소·거절** — 스키마에 거절 시각 컬럼이 없어(부록 D 의 ERD 에 없다)
  **행을 지울지 컬럼을 늘릴지부터** 정해야 한다. 이것이 없어서 위 409 를 풀 수단이
  없다
...
- 🔴 **취소·거절** — 지원을 물리거나 팀이 거절하는 경로. 스키마에 거절 시각 컬럼이
  ...
  **이것이 없어서 지원이 붙은 경기를 취소할 수 없다**(3-4절 `DELETE /matches/{id}`
  의 409) — 두 문제가 같은 결정에 걸려 있다
