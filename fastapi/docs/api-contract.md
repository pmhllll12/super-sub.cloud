...
**409 가 오면 지원을 먼저 정리해야 한다.** 그런데 지원을 무르는 경로가 아직 없다
(아래 「아직 없는 것」) — **지금은 지원이 붙으면 취소할 방법이 없다.** 이것을 어떻게
풀지는 미결 항목으로 올려 두었다.

### 아직 없는 것

- 🔴 **지원 취소·거절** — 팀이 지원을 정리할 경로가 없어서 위 409 를 풀 수단이 없다.
  **거절을 행 삭제로 할지 컬럼(`team_rejected_at`)으로 할지**를 정해야 한다.
  🔴 **컬럼으로 하면 행이 남아 409 가 그대로다** — 즉 이 결정은 "무엇을 기록할까"가
  아니라 **"409 를 풀 것인가"** 다. 상세와 선택지는 미결 `jin` 16번
...
- 🔴 **취소·거절** — 지원자가 물리거나 팀이 거절하는 경로. **이것이 없어서 지원이
  붙은 경기를 취소할 수 없다**(3-4절 `DELETE /matches/{id}` 의 409) — 두 문제가
  같은 결정에 걸려 있다.

  ⚠️ **지원자가 무르는 것만으로는 409 가 안 풀린다.** 그건 행 삭제라 ERD 와
  무관하고 어차피 필요하지만, 팀이 취소하려면 **지원자 전원이 스스로 물러 주어야**
  하므로 해결이 못 된다. **팀 쪽에서 정리하는 경로**가 있어야 하고, 그때 거절을
  **행 삭제로 할지 컬럼으로 할지**가 갈린다 — **컬럼이면 행이 남아 409 가 그대로다.**
  선택지 넷(A-1·A-2·B·C)은 미결 `jin` 16번
