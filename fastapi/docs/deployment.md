...
> **주소 이름은 `api.supersub-ai.com` 으로 정해 두었다**(2026-09-02). 다만
> **A 레코드는 아직 만들지 않는다** — 탄력적 IP 를 안 붙이기로 해서 인스턴스를
> 껐다 켜면 주소가 바뀌기 때문이다(8절). 도메인·인증서·포트는 **외부 공개를
> 결정하는 날 한꺼번에** 한다. 지금은 SSH 터널로 쓴다(6-6 절).
...
| 🔴 **업로드용 S3 버킷** | 2026-09-03 에 클립 업로드(SFR-001)를 S3 로 정했지만 **버킷이 아직 없다.** 없으면 업로드 경로가 통째로 503 이다 — 아래 9절 |
...
🔴 **탄력적 IP 를 붙이지 않기로 했다**(2026-09-02 사용자 결정). 그래서 **켤 때마다
공인 IP 가 바뀐다** — 접속 설정을 그때마다 고쳐야 하고, **DNS A 레코드는 지금
만들면 안 된다**(곧 틀어진다). 도메인은 외부 공개를 결정하는 날 함께 만든다.

### 껐다 켠 뒤에 할 일 (매번)

탄력적 IP 가 없으므로 **주소가 바뀐다.** 켠 뒤 이 순서로 한 번 정리한다.
...
## 9. 업로드용 S3 버킷 (2026-09-03 — **아직 안 만들었다**)

> **상태:** 진행 전 · 2026-09-03 확인
> **확인:** `aws s3api head-bucket --bucket <이름>` → `404` 면 미착수
> **메모:** 코드는 다 들어갔다. 버킷과 권한만 남았다
