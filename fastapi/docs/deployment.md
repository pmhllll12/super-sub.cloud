...
| 상황 | 추가 비용 |
|---|---|
| 인스턴스가 **켜져 있을 때** | **거의 0.** 2024-02 부터 AWS 는 **모든 퍼블릭 IPv4** 에 시간당 $0.005 를 매긴다. 지금도 인스턴스 공인 IP 에 그 요금이 나가고 있고 탄력적 IP 가 그것을 대체한다 |
| 인스턴스를 **꺼 뒀을 때** | 미사용 탄력적 IP 로 시간당 $0.005 — **월 약 $3.6** |

두 달 $400 한도에서 월 $3.6 이면 1% 미만이고, **켤 때마다 접속 설정을 고치는
비용이 그보다 크다.** 09-02 에는 "인스턴스가 꺼져 있는 동안 과금된다"만 보고
안 붙이기로 했는데, **켜져 있는 동안에도 어차피 같은 요금이 나간다**는 쪽을
빠뜨린 판단이었다.
...
- 탄력적 IP 는 **인스턴스가 꺼져 있는 동안에도 과금**된다(월 약 $3.6). 켜져 있는
  동안은 인스턴스 공인 IP 요금을 대체하므로 추가 비용이 거의 없다 — 위 표 참고
...
| 8K 로 등록 | 당시(09-03) `passed: false` · 사유 `"해상도가 상한을 넘습니다: 7680x4320 (상한 긴 변 3840 · 짧은 변 2160)"` — 2026-09-11 정정: 4K(3840x2160)까지는 `passed: true`가 됐고, 🔴 **2026-09-19 재정정**: 해상도 상한 자체가 없어져 8K도 이제 `passed: true`다 |
...
| | |
|---|---|
| 버킷 | **`supersub-ai`** (서울 `ap-northeast-2`, 09-02 생성) |
| 접두사 | **`videos/`** — 09-03 확인 시점에 비어 있었다 |
| 설정 | `S3_BUCKET=supersub-ai` · `AWS_REGION=ap-northeast-2` |

이 버킷은 **프로젝트 공용**이고 세 갈래로 나뉘어 있다.

| 접두사 | 무엇 | 누구 |
|---|---|---|
| `models/` | 판정 모델 자산 | 정상호 |
| `reports/` | 분석 리포트 산출물 | 정상호 |
| **`videos/`** | **사용자가 올린 클립** | 백엔드(이 문서) |

코드가 만드는 키가 `videos/<user_id>/<uuid>.mp4` 라 접두사가 그대로 맞는다
(`app/analysis/domain/rules/video_rules.py`). **`videos/` 밖에는 쓰지 않는다.**

🔴 **새로 만들기 전에 있는 것을 먼저 본다.** 09-03 에 "버킷이 없다"고 적었다가
콘솔에 이미 있는 것을 확인하고 고쳤다. 그리고 **`videos/` 접두사까지 이미 있었다** —
"남의 모델용 버킷"이 아니라 처음부터 공용으로 설계된 것이었다.

```bash
aws s3 ls                        # 만들기 전에 이것부터
aws s3 ls s3://supersub-ai/      # 접두사 구조도 함께 본다
```

### 접두사로 나눌 수 있는 것과 없는 것

같은 버킷을 나눠 쓰므로 **어디까지 격리되는지**를 알고 있어야 한다.
...
플러터 앱은 직접 PUT 하므로 상관없지만, `www` 에서 올리려면 버킷에 CORS 규칙이
있어야 한다(`PUT` 허용, `Origin` 은 웹 배포 주소). **없으면 브라우저가 요청을 막고
서버 로그에는 아무것도 안 남는다** — 원인을 찾기 어려운 자리다.

🔴 **CORS 는 버킷 전체에 걸리므로 정상호에게 알리고 넣는다.**

### 아직 안 정한 것

- **보관 기간.** 클립을 얼마나 두는지가 안 정해졌다. 정해지면 `videos/` 접두사에
  수명 주기 규칙 하나로 붙는다
- **공개 접근·버전 관리**는 버킷을 만든 쪽(정상호)이 정한 값을 따른다. 원본은
