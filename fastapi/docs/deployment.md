...
## 9. 업로드용 S3 버킷 (2026-09-03)

> **상태:** 진행중 · 2026-09-03 확인
> **확인:** `aws s3api head-bucket --bucket <이름>` → `404` 면 그 버킷이 없다
> **메모:** 코드는 다 들어갔다. **계정에 버킷은 이미 있고**(서울 리전, 09-02 생성)
> 클립용으로 어느 것을 쓸지·접두사를 어떻게 나눌지가 안 정해졌다. `S3_BUCKET` 이
> 서버 `.env` 에 아직 없어 업로드 경로는 503 이다

🔴 **새로 만들기 전에 있는 것을 먼저 본다.** 09-03 에 "버킷이 없다"고 적었다가
콘솔에 이미 둘 있는 것을 확인하고 고쳤다.

```bash
aws s3 ls    # 만들기 전에 이것부터
```

**남의 용도로 만든 버킷에 클립을 섞지 않는다** — 수명 주기·권한·비용이 용도마다
다르고, 한번 섞으면 나중에 가르는 것이 이사(移舍)가 된다. 같은 버킷을 쓰더라도
접두사(`videos/`)로 나누고 그것을 문서에 적는다.
...
| 항목 | 값 | 왜 |
|---|---|---|
| 리전 | EC2 와 같은 곳 | 다르면 전송 요금이 붙고 지연이 는다 |
| 공개 접근 | **전부 차단** | 원본은 사전 서명 URL 로만 오간다. 공개하면 남의 클립을 URL 추측으로 볼 수 있다 |
| 버전 관리 | 끔 | 같은 키에 덮어쓰지 않는다(키마다 uuid) |
| 수명 주기 | 나중에 | 보관 기간이 안 정해졌다. 정해지면 규칙 하나로 붙는다 |

### 서버에 줄 권한

EC2 인스턴스 역할에 **그 버킷의 객체에 대한 `PutObject`·`GetObject`·`HeadObject`**
만 주면 된다. 🔴 **장기 액세스 키를 서버 `.env` 에 두지 않는다** — boto3 가 인스턴스
