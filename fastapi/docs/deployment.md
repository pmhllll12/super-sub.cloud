...
> **확인:** 0절의 확인 명령을 돌린다 — **항목별로 찍는다.** `command -v a b` 는 하나만 찾아도 0 이라 미착수 판별에 못 쓴다.
> 파이썬은 2026-09-02 에 넣었고 나머지는 아직이다.
...
**⑴ DB 를 어디에 두는가.** RDS(관리형·`pgvector` 는 `rds_superuser` 로 생성)와 같은
