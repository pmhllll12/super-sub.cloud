...
**아래 「패킷 B」 절은 지시가 아니라 기록으로 읽어 주십시오.**

| 단계 | 누가 |
|---|---|
| 스키마 (마이그레이션 · `review_option` 초기 9개) | **박민호** (09-03, `2649dd9`) |
| ORM 다섯 · `alembic/env.py` 등록 | 정어진 (09-04, `f97d72b`) |
| 어댑터 · 라우터 · 테스트 | 정어진 (09-04, `f6d09ba`) |
| 공유 파일 5곳 배선 | 정어진 |

```bash
git grep -n "review_router" -- fastapi/app     # 결과가 있으면 끝난 것입니다
```

**박민호 님이 안 하신 것이 아닙니다.** 초기 목록 확인을 기다리며 멈춰 계셨는데,
그 사이 마이그레이션만 `main` 에 들어가 **CI 가 깨졌고**(아래 「마이그레이션만
먼저 가면」) 그걸 고치다 제가 끝까지 갔습니다.
...
| 평가 가능 기간 | 경기 후 **14일** |
| 불참 기록 권한 | **주최 팀 주장만** |
| 선택지 노출 순서 | **`sort_order` 컬럼** (부록 D 갱신 필요 — 미결 `jin` 19번) |

신고 `reason` 은 마이그레이션이 낸 `Text` 그대로 자유 텍스트입니다.

규격은 `docs/api-contract.md` **3-9절**, 클라이언트 반영은
`docs/client-contract-changes.md` **16번**입니다.
...
## ~~패킷 B — 평가·신뢰 (박민호 님)~~ ✅ 끝났습니다 (2026-09-04)

> **지시가 아니라 기록입니다.** 위 「패킷 B 는 끝났습니다」 절을 먼저 보십시오.
> 아래는 무엇을 왜 그렇게 만들었는지가 남아 있어 지우지 않았습니다.
