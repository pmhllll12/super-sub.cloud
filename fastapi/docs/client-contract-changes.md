...
## 59. 경기 신청 한 줄에 **두 팀 스쿼드의 공개 슬러그**가 실립니다 (2026-09-17 추가, `paik` 22번 후속)

대기 화면이 **상대 팀 판을 진짜로 그릴 수 있게** 됐습니다. 전에는 화면이
붙박이 목록(`www/src/lib/teamMatch.ts` 의 `TEAMS`)에서 상대 팀 이름·판을
찾았고, 그 목록에 없는 진짜 팀이 수락하면 **이름이 「상대 팀」으로 나오고
판이 비었습니다.**
...
🔴 **기존 칸은 자리가 안 바뀝니다** — 감싸지 않았습니다. 늘어난 둘만 더
읽으면 됩니다. 생성·목록·수락·거절·무르기 **다섯 경로가 같은 모양**이라
파서 하나로 읽힙니다.

### 만족해야 할 성질

`GET /squads/{public_slug}` 로 그 판을 그릴 수 있을 것 — **인증이 필요
없습니다**(SEC-005). 소속이 아니어도 읽히는 것이 이 값을 싣는 이유입니다.

### 🔴 하지 말 것

- **`null` 을 실패로 보지 마십시오** — 스쿼드를 아직 안 만든 팀이 정상입니다
  (생성이 멱등이라 늦게 생깁니다). 그때는 판 대신 「아직 판이 없습니다」로.
- **붙박이 목록으로 되돌아가지 마십시오** — 이 값이 그것을 걷으려고 생겼습니다.

### 확인

```bash
# 늘어난 칸이 나가는가 (로그인 토큰 필요)
curl -s -H "Authorization: Bearer $TOKEN" \
  "$API/api/v1/teams/<내 팀>/match-requests" | jq '.[0]'
```

🔴 **마이그레이션 0건** — 응답 필드만 늘었습니다. 상세: `api-contract.md`
(3-15절 「두 팀 스쿼드의 공개 슬러그」)
...
## 62. 같은 상대에 **겹쳐 걸면 409** 입니다 — 「끝냈는데 또 그 화면이 뜬다」의 원인 (2026-09-18 추가)

`POST /teams/{team_id}/match-requests` 에 새 에러가 하나 생겼습니다.

| 에러 | code |
|---|---|
| **409** | **`TEAM_MATCH_REQUEST_ALREADY_LIVE`** — 그 상대와 이미 살아 있다 |

「살아 있다」 = 그 두 팀 사이에 `pending`(답을 기다림)이거나, `accepted` 이면서
**경기가 아직 있고**(`match_id` 가 차 있고) **아직 안 지난** 신청이 있다.
**거절·취소된 것, 이미 지난 경기, 물린 경기는 세지 않습니다** — 안 그러면 한 번
붙은 팀과 다시는 못 붙습니다.

> 🔴 **물린 경기를 가려내는 것은 `status` 가 아니라 `match_id` 입니다.** 경기
> 취소는 `match` 행만 지우고 신청의 `status` 는 `accepted` 로 남습니다 —
> 「경기 잡힘」 화면이 쓰는 조건과 같습니다.

🔴 **방향을 가리지 않습니다.** A→B 가 살아 있으면 B→A 도 막힙니다.
**여러 팀에 동시에 거는 것은 여전히 됩니다** — 막는 것은 *같은 상대에 두 번*입니다.

**왜 생겼나.** 수락이 두 팀의 다른 `pending` 을 정리하지만(계약 3-15절
「동시 확정 방지」) 그건 `pending` 까지입니다. 이미 `accepted` 가 된 뒤 같은
상대에 새로 거는 것은 서버가 막지 않았고, 화면의 「한 번에 한 곳」
(`TeamMatch.tsx`)은 **그 브라우저의 상태**뿐이라 새로고침하면 그대로
뚫렸습니다. 운영에서 한 팀이 같은 상대와 **확정 경기 3건**을 갖게 됐고,
경기를 하나 완료하거나 취소할 때마다 **다음 것이 곧바로 올라와** 사용자에게는
「완료했는데 새로고침하면 그 화면이 또 남는다」로 보였습니다.

### 만족해야 할 성질

- 409 를 **다른 실패와 구분해서 사람 말로 보여 주기.** 이미 `applyToTeam` 이
  서버 `error.message` 를 그대로 띄우므로 (`'이미 이 팀과 진행 중인 신청이나
  잡힌 경기가 있습니다.'`) **그대로 두어도 성질은 만족합니다** — 더 나은 문구를
  쓰고 싶으면 `code === 'TEAM_MATCH_REQUEST_ALREADY_LIVE'` 로 갈라 쓰십시오
- 409 를 받았을 때 **「탐색 중」 상태를 끄지 않기.** 실패가 아니라 *이미 되어
  있다*는 뜻이므로, 그 팀 줄만 「이미 신청함」으로 두고 나머지는 계속 고를 수
  있는 편이 맞습니다

### 먼저 확인

```bash
grep -n 'applyToTeam' -A 25 www/src/lib/teamMatch.ts | grep -n 'error?.message'
```

걸리면 **서버 문구가 이미 화면에 뜹니다 — 손대지 않아도 됩니다.**

### 하지 말 것

- 🔴 **화면에서 중복을 다시 계산하지 마십시오** — 「살아 있음」 판정은 서버가
  하는 일이고, 화면이 자기 목록으로 흉내내면 다른 기기에서 건 것을 못 봅니다
- 🔴 **409 를 「신청 실패」로 취급해 자동으로 재시도하지 마십시오** — 영영
  안 풀립니다. 사람이 상대를 바꾸거나 기존 경기를 끝내야 풀리는 상태입니다

- 확인: 백엔드 `tests/match/adapter/test_team_match_request_router.py` 의
  `TestDuplicateGuard` (6건, 고치기 전 3건 빨강 확인) ·
