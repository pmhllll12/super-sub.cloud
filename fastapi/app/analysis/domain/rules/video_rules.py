...
| 계정당 개수 | **3개** (2026-09-22 신설 — 아래 참고) | 저장 비용·분석 비용을 계정 단위로 묶는다 |

🔴 **계정당 개수 상한은 다른 둘과 성질이 다르다 (2026-09-22, 사용자 요청)**:
용량·길이는 **클립 하나**를 보고 판단하므로 `reject_reason` 이 사유 문장으로
답할 수 있지만, 개수는 **계정이 이미 가진 것**을 봐야 해서 이 모듈이 혼자
판단할 수 없다(여기엔 DB 가 없다). 그래서 여기엔 값(`MAX_VIDEOS_PER_USER`)만
두고 판단은 인터랙터가 한다.
...
#: 한 계정이 가질 수 있는 **저장된**(`kept=true`) 영상 수. 위 두 상한과 달리
#: 클립 하나를 재는 값이 아니라 **계정 전체를 재는 값**이라, 쓰는 쪽이 저장소에
#: 개수를 물어봐야 한다(`VideoPort.count_kept_by_user`).
