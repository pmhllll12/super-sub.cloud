...
| 개수 | **갈래마다 3개** (2026-09-22 신설 — 아래 참고) | 쌓이는 양을 계정 단위로 묶는다 |

🔴 **개수 상한은 다른 둘과 성질이 다르다 (2026-09-22, 사용자 요청)**:
용량·길이는 **클립 하나**를 보고 판단하므로 `reject_reason` 이 사유 문장으로
답할 수 있지만, 개수는 **계정이 이미 가진 것**을 봐야 해서 이 모듈이 혼자
판단할 수 없다(여기엔 DB 가 없다). 그래서 여기엔 값(`MAX_VIDEOS_PER_GROUP`)만
두고 판단은 인터랙터가 한다.

🔴 **「갈래마다」다 — 합쳐서 3개가 아니다 (2026-09-22 사용자 결정으로 정정).**
처음엔 둘을 합쳐 3개로 넣었는데, 화면(`www` 의 「내 영상」)이 **분석 영상 /
업로드 영상** 두 탭으로 갈라 보여주고 있어서 합치면 한쪽이 다른 쪽을 막는다.
**갈래를 가르는 기준은 화면과 같은 `analysis_job_id` 유무**다 — 분석 작업이
걸린 클립은 「분석 영상」, 안 걸린 것(기록용 업로드·반려·중복 재사용)은
「업로드 영상」. 그래서 한 계정의 최대는 **3 + 3 = 6개**다.

🔴 **그래서 개수 초과는 반려가 아니라 422 다.** 위 「반려는 실패가 아니다」
(`video_interactors.py` 머리말)가 말하는 반려는 "등록은 됐고 분석 대상이 아닐
뿐"인데, 개수 초과는 **등록 자체가 성립하지 않는다** — 자리가 없다. 반려로
기록하면 그 반려 행이 또 한 자리를 차지해 상황이 더 나빠진다.
...
#: 한 계정이 **갈래마다** 가질 수 있는 **저장된**(`kept=true`) 영상 수.
#: 갈래는 화면의 두 탭과 같다 — 분석 영상(분석 작업이 걸린 것) / 업로드 영상.
#: 위 두 상한과 달리 클립 하나를 재는 값이 아니라 **계정을 재는 값**이라,
#: 쓰는 쪽이 저장소에 개수를 물어봐야 한다(`VideoPort.count_kept_by_user`).
MAX_VIDEOS_PER_GROUP = 3
