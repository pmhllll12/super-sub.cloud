...
## 12. 계획 1 완료 후 이월 항목 (2026-08-25)

계획 1(토대와 첫 플로우)이 완료되었다. 전체 브랜치 리뷰와 수정 웨이브를 거치며 나온 것 중,
**계획 2 착수 시 먼저 처리해야 할 것**을 우선순위대로 남긴다.

| # | 항목 | 왜 먼저인가 |
|---|---|---|
| 1 | **콜드 스타트 시 `/home`이 먼저 그려진다** | `initialLocation`이 `/home`이고 복원 중(`SessionUnknown`)에는 리다이렉트하지 않으므로, 앱을 켤 때마다 인증된 홈 화면이 한 번 칠해진 뒤 `/login`으로 넘어간다. Mock 저장소가 프로세스 로컬이라 **매 콜드 스타트에서 반드시 발생**한다. 5.1이 말한 대로 `/splash`를 라우팅 분기로 두고 `SessionUnknown` 동안 로딩을 그리는 것이 정공법. 라우터에 새 라우트를 추가하기 전에 처리한다 |
| 2 | **`FutureProvider`의 자동 재시도 정책** | Riverpod 3은 실패한 provider를 자동 재시도하며 `AsyncLoading`으로 붙잡아, `AsyncView`의 오류 분기가 **영원히 렌더되지 않는다**. 현재 `sportsProvider`에만 `retry: (_, _) => null`을 걸어 막았다. 루트 `ProviderScope`의 기본값으로 두면 각자 컨테이너를 만드는 위젯 테스트에 적용되지 않아 lib과 test가 어긋난다. **공유 상수를 만들어 provider마다 적용**하거나, `AsyncView`가 `isLoading`보다 `hasError`를 우선하도록 고쳐 한 파일에서 해결한다 |
| 3 | **위젯 테스트 공유 헬퍼** | 가짜 시계 함정(Mock `Future`를 `pumpWidget` 전에 await하면 10분 타임아웃까지 멈춤)이 개발 중 세 번 재발했다. 관용구가 테스트 5개 파일에 주석으로 흩어져 있고 설명이 서로 어긋난다. 규칙 두 개(① `pumpWidget` 전에 Mock `Future`를 await하지 않는다 ② 스피너가 떠 있는 동안 `pumpAndSettle`을 쓰지 않는다)를 담은 헬퍼를 한 곳에 만든다 |
| 4 | **`loginAs`의 위치** | `loginAs`는 `kDebugMode` 개발 편의 기능인데 `AuthRepository`의 추상 멤버로 남아 있어, 미래의 `ApiAuthRepository`가 구현하거나 던지도록 강요된다. 공유 계약에서는 이미 뺐으므로 인터페이스도 맞춰야 한다(예: 개발 전용 mixin 분리) |
| 5 | `AsyncView`의 오류 표시 | 현재 `'$e'`를 그대로 그린다. 첫 실제 소비처에서 사용자에게 예외 `toString()`이 노출되지 않도록 호출부에서 메시지를 구성한다 |
| 6 | 종목 선택 위젯 중복 | 온보딩(카드)과 홈(바텀시트)에 두 번 구현되어 있고 둘 다 현재 선택을 표시하지 않는다. 공유 위젯으로 합치면서 홈의 시트가 부모 `WidgetRef`를 클로저로 잡는 것도 함께 정리한다 |
