...
/// 🔴 **순검정이다 (2026-09-24, 같은 날 세 번째. 사용자 요청: 「배경색상 완전
/// 검정으로 하고, 그 내 프로필 있는 맨 위 판의 색상을 지금 배경색상으로 다시
/// 바꾸자」).** 그래서 **바탕이 `#000000`, 판이 `#222021`** 이다 — 몇 시간 전
/// 맞바꿔 둔 것(바탕 `#222021` · 판 `#E4E9E7`)에서 **또 옮긴 것**이니 주의해서 읽을 것.
///
/// ⚠️ **판과의 밝기 차이가 아주 작다**(휘도 0 대 0.015). 어두운 회색 판이
/// 검은 바탕 위에 **은은하게만** 뜨는 것이 의도다 — 「판이 잘 안 보인다」는
/// 이유로 둘 중 하나를 밀지 말 것. 또렷하게 하려면 **판에 테**를 주는 쪽으로 간다.
/// (그 탓에 `home_screen_test.dart` 의 「판과 바탕의 밝기 차 0.3」 단언을
/// **「색이 다르다」로 낮췄다** — 판 위 글자를 같이 안 돌린 것은 같은 파일의
/// 글자색 단언이 대신 잡는다.)
///
/// 🔴 **판 위 글자는 다시 흰색이다** — `home_screen.dart` 의 `_kOnPanel`.
/// 판이 어두워졌으므로 `_kOnDark`(스쿼드 판·사진 위)와 값이 같아졌지만,
/// **상수는 계속 갈라 둔다**(판만 또 뒤집힐 수 있다).
///
/// 🔴 색 이력: 흰색(`#F6FAF5`) → 순검정 → `#131213` → 밝은 회색(`#EFEFEF`) →
/// 딥그린(`#0F2E1F`) → 파스텔 딥그린(`#38483D`) → 라이트그레이(`#E4E9E7`) →
/// `#222021` → **순검정**.
///
/// ⚠️ **초록 계열을 아예 벗어났다.** `#38483D` 까지는 [kCardBg](`#91EA92`)와
/// 계열을 맞추는 규칙이 살아 있었지만 그 뒤로는 **레퍼런스 그림**에서 온
/// 무채색이다. 「앱 초록을 밝게/어둡게 옮겨라」로 되돌리지 말 것.
///
/// ⚠️ **지름길 알약은 이 값을 안 쓴다** — `home_screen.dart` 의 `_kPillFill`
/// 이 갈라져 나갔다(알약은 **흰 판 위**라 바탕을 따라가면 안 된다).
/// 알약 색을 바꾸려면 그쪽을 본다.
