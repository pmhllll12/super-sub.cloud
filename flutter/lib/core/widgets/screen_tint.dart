...
/// 🔴 **딥그린이다 (2026-09-23 다섯 번째 정정, 사용자 요청: 「딥그린으로
/// 바꿔볼래?」).** 하루 사이 흰색(`#F6FAF5`) → 순검정 → `#131213` →
/// 밝은 회색(`#EFEFEF`) → **이 값**으로 왔다.
///
/// 🔴 **[kCardBg](`#91EA92`)를 그대로 어둡게 내린 값이다** — 같은 색상각
/// (120°)을 지키고 명도만 떨어뜨렸다. 아무 초록이나 고르면 앱의 초록과
/// **미묘하게 다른 계열**이 되어 카드·로고와 어긋나 보인다.
///
/// ⚠️ **어두운 쪽이라 흰 글자가 다시 산다** — 밝은 회색이던 동안 안 보이던
/// 「내 프로필」·흰 판 경계 문제가 이 값에서는 없다. 다시 밝은 쪽으로 갈
/// 일이 있으면 그 둘을 같이 봐야 한다.
const Color _kMintBase = Color(0xFF0F2E1F);
