...
class MockDb {
  ...
  void _seed() {
    ...
    /* 🔴 **꾸며진 채로 시드한다 (2026-09-22, 사용자 요청).** 전에는 `style`
       ...
       🔴 **사용자가 폰에서 꾸민 카드를 그대로 옮긴 것이다** (2026-09-22 갱신).
       전에는 검은 바탕 · 붉은 나선(파티클 플럭스)이었고, 지금은 **하늘색
       바탕 · 크림색 글자 · 크림색 수채 구름**이다. 값은 사용자가 불러 준
       것이고(색 셋 · 배율 · 좌우 · 위아래), 화면을 픽셀로 떠서 맞는지
       확인했다.
    ...
    cards.add(PlayerCard(
      ...
      style: CardStyle.fromJson(const {
        ...
        'text_color': '#FFF4DA',
        // 🔴 **사용자가 안 불러 준 유일한 값이라 화면에서 맞췄다.** 23.0 으로
        // 넣고 찍어 보니 원본보다 7물리px(카드 높이의 0.98%) 높아서 24.0 으로
        // 올렸다 — 다시 찍어 확인했다.
        'text_y': 24.0,
        'brush': 11,
        'brush_color': '#FFF0CC',
