...
class MockDb {
  ...
  void _seed() {
    ...
    /* 🔴 **꾸며진 채로 시드한다 (2026-09-22, 사용자 요청).** 전에는 `style`
       ...
       🔴 **사용자가 폰에서 꾸민 카드를 그대로 옮긴 것이다** (2026-09-22에
       두 번 갱신). 검은 바탕 · 붉은 나선 → 하늘색 바탕 · 크림 수채 구름 →
       지금은 **더 밝은 하늘색(`#73C6FF`) 바탕에 글자 · 워드마크 · 자국이
       모두 순백**이고 글자는 `USER` 다.

       ⚠️ **값은 전부 사용자가 불러 준 것이다** — 화면에서 역산하지 않았다.
       앞 회차에 `text_y` 하나를 그림 대조로 맞추느라 두 번 고쳤는데, 불러
       주는 쪽이 언제나 빠르고 정확하다.
       ...
       🔴 **`brush: 11` 이 수채 구름이다** — 자국 번호는 `파일 번호 + 1` 이고
       그 그림이 `assets/marks/10.png` 다. 목록에서 세지 말 것(숨긴 자국
       둘 때문에 자리와 번호가 어긋난다). */
    cards.add(PlayerCard(
      ...
      style: CardStyle.fromJson(const {
        'bg': '#73C6FF',
        // 🔴 하나가 글자·워드마크·머리글을 함께 움직인다 — 그래서 「워드마크도
        //    흰색」은 이 한 줄로 끝난다.
        'text_color': '#FFFFFF',
        'text_x': 50.0,
        'text_y': 24.0,
        // 🔴 **자국은 그대로 11(수채 구름)** — 색만 순백으로 갈았다.
        'brush': 11,
        'brush_color': '#FFFFFF',
        'brush_scale': 1.83,
        'brush_x': -4.0,
